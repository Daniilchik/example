...
```bash
# Используй тот же image что в production
...
oc run knox-debug \
  ...
  --overrides='{"spec":{"securityContext":{"runAsUser":10001,"runAsGroup":10001,"fsGroup":10001}}}' \
  --command -- sleep 3600

# Дождаться Running
oc get pod knox-debug -w
# Ctrl-C когда Running

# Зайти внутрь
oc exec -it knox-debug -- /bin/sh
```
...
```bash
oc create secret generic knox-master \
